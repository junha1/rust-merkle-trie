...
pub type ProofCreation<H, K, V> = (CryptoProofUnit<H, K, V>, Box<dyn CryptoProvable<H, K, V>>);
pub trait CryptoStructure<H, K, V> {
    fn make_proof(&self, key: &K) -> crate::Result<ProofCreation<H, K, V>>;
}
...
pub struct CryptoProofMerkleTrie {
    pub proof: Vec<Bytes>, // Starts with the closest node to root.
}

impl CryptoProvable<H256, H256, Bytes> for CryptoProofMerkleTrie {
    // Proof should start with root node.
    fn verify(&self, test: &CryptoProofUnit<H256, H256, Bytes>) -> bool {
        type Unit = CryptoProofUnit<H256, H256, Bytes>;
        // step1: verify the value
        fn step1(self_: &CryptoProofMerkleTrie, test: &Unit) -> bool {
        ...
        fn step2(self_: &CryptoProofMerkleTrie, test: &Unit) -> bool {
            blake256(&self_.proof[0]) == test.hash
        };

        // step3 (presence): verify the key
        fn step3_p(self_: &CryptoProofMerkleTrie, test: &Unit) -> bool {
            ...
            verify_branch(&NibbleSlice::new(&test.key), &test.hash, &self_.proof)
        };

        // step3 (absence): verify the key.
        fn step3_a(self_: &CryptoProofMerkleTrie, test: &Unit) -> bool {
            fn verify_branch(path: &NibbleSlice<'_>, hash: &H256, proof: &[Bytes]) -> bool {
                if *hash != blake256(&proof[0]) {
                    return false
                }
        ...
        if self.proof.is_empty() {
            return test.hash == BLAKE_NULL_RLP && test.value.is_none() // special case of an empty trie.
        }
...
mod tests {
    ...
    type ProofUnit = CryptoProofUnit<H256, H256, Bytes>;
